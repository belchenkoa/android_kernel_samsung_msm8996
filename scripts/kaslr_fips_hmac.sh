...
# These are native host utilities. Android 11's prebuilt clang does not use
# the system GNU ld; select the LLD shipped with the clang toolchain explicitly.
# Do not gate this on -x: ld.lld may be a symlink and the previous probe caused
# clang to fall back to looking for a plain "ld" in PATH.
HOSTCC_LDFLAGS=""
HOSTCC_BIN="${HOSTCC%% *}"
